...
from rapidfuzz.distance.JaroWinkler import normalized_similarity as rf_jw_sim
...
start = perf_counter()
for idx in range(N_ITERS):
    sim_rust = rf_jw_sim(x, y)
end = perf_counter()
print(f'Rapidfuzz Jaro winkler Elapsed time:    {end - start} seconds')

print(81 * "=" + "\n")

vectorized_python_jw = np.vectorize(jaro_winkler_similarity_python)
vectorized_rust_jw = np.vectorize(jaro_winkler_similarity)
vectorized_rapidfuzz_jw = np.vectorize(rf_jw_sim)
x_s = pd.Series([x] * N_ITERS)
y_s = pd.Series([y] * N_ITERS)

start = perf_counter()
sim_python = vectorized_python_jw(x_s, y_s)
end = perf_counter()
print(f'Vectorized python Jaro Winkler time:               {end - start} seconds')


start = perf_counter()
sim_rust = vectorized_rust_jw(x_s, y_s)
end = perf_counter()
print(f'Vectorized rust Jaro winkler Elapsed time:         {end - start} seconds')

start = perf_counter()
sim_rapidfuzz = vectorized_rapidfuzz_jw(x_s, y_s)
end = perf_counter()
print(f'Vectorized rapidfuzz Jaro winkler Elapsed time:    {end - start} seconds')
...
print(81 * "=" + "\n")
'''
del str1_list, str2_list, sim_rust
gc.collect()
...
for idx in tqdm(range(N_ITERS)):
    sim_rust_jaccard = get_jaccard(x_s[idx], y_s[idx])
end = perf_counter()
...
start = perf_counter()
for idx in range(N_ITERS):
    sim_python_jaccard = jaccard_similarity_python(x_s[idx], y_s[idx])
end = perf_counter()
